...
(function () {
  ...
  if (
    ...
    ['require', 'module', 'process', 'global', '__dirname'].forEach(function (it) {
      window[it] = window.top[it]
    })

    // Rewrite default console on console patched by karma
    window.top.global.console = console

    // Clear cache to pick up changes in required modules
    // This is need only for iFrame and watch mode
